...
def _builtin_generate(config_path: Path, config: dict, messages: list[dict], tools_schema: list[dict]) -> str:
    try:
        import openai
        import json
    except ImportError:
        raise RuntimeError("builtin mode requires openai package")
    base_url = config.get("model", {}).get("base_url", "http://localhost:8000/v1")
    api_key = config.get("model", {}).get("api_key", "EMPTY")
    client = openai.OpenAI(base_url=base_url, api_key=api_key)
    model_name = config.get("model", {}).get("model_name_or_path", "Qwen3.5-4B")
    response = client.chat.completions.create(
        model=model_name,
        messages=messages,
        tools=tools_schema,
        temperature=config.get("generation", {}).get("temperature", 0),
        max_tokens=config.get("generation", {}).get("max_new_tokens", 1024),
        top_p=config.get("generation", {}).get("top_p", 1)
    )
    tool_calls = []
    ...
    ai_message = {"role": "assistant", "content": content, "tool_calls": tool_calls}
    raw_text = json.dumps(ai_message, ensure_ascii=False)
    return raw_text
def _prompt_json_generate(config_path: Path, config: dict, messages: list[dict], tools_schema: list[dict], model_override: str = None) -> str:
    try:
        import torch
...
def generate_ai_message(
    ...
    if model_override is not None:
        config["model"]["model_name_or_path"] = model_override
    ...
    if mode == "mock":
        ...
        raw_text = _prompt_json_generate(config_path, config, messages, tools_schema, model_override=model_override)
        try:
        ...
        raw_text = _builtin_generate(config_path, config, messages, tools_schema)
        try:
            parsed_candidate, ai_message = _parse_model_output(raw_text)
            status = "success"
            error = None
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mode", choices=["mock", "prompt_json", "builtin"], required=True)
    parser.add_argument("--outdir", required=True)
    parser.add_argument("--model", type=str, default=None, help="Override model name/path")
    return parser
